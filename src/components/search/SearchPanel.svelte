<style>
  .flyout {
    ...
    grid-template-rows: 74px 46px auto 1fr 45px auto;
    border-radius: 2px;
  ...
  :global(.ais-SearchBox) {
    ...
    grid-template-areas: '. search-icon clear-icon';
    -ms-grid-columns: 1fr 36px 36px;
  ...
  :global(.ais-SearchBox-input:hover) {
    box-shadow: inset 0 0 0 2px var(--color-neutral-dark);
  }
  ...
  :global(.search-flyout--hit-card-link-block) {
    text-decoration: none;
  }
  :global(.search-flyout--hit-card) {
    margin-right: 10px;
    margin-left: 10px;
    padding: 16px 2px;
  ...
  :global(.search-flyout--hit-card:hover) {
    color: #777;
  }
  :global(.search-flyout--hit-card--tags) {
    display: flex;
    font-family: Lato, sans-serif;
    font-size: 12px;
    line-height: 20px;
  ...
  :global(.search-flyout--hit-card--name) {
    font-family: 'Adamant BG', Georgia, sans-serif;
    font-size: 18px;
    line-height: 24px;
  }
  :global(.search-flyout--hit-card--description) {
    font-family: Georgia, Times, 'Times New Roman', serif;
    font-size: 14px;
    line-height: 20px;
  }
  :global(.search-flyout--hit-card--metadata) {
    font-family: Lato, sans-serif;
    color: #777;
    font-size: 14px;
    line-height: 20px;
  ...
  :global(.ais-Pagination-item:hover) {
    background-color: var(--button-bg-hover);
    border: 1px solid var(--button-bg-hover);
    box-shadow: var(--button-shadow-hover);
  }
  :global(.ais-Pagination-item:active) {
          ...
          item(item: IArticle) {
            ...
            return `
              <a href="https://thepaladin.news/articles/${
                ...
                <div class="search-flyout--hit-card">
                  ${
                    categories.length !== 0
                  ...
                  <div class="search-flyout--hit-card--name">${item.name}</div>
                  <div class="search-flyout--hit-card--description">${item.description}</div>
                  <div class="search-flyout--hit-card--metadata">
...
<div class={'flyout'}>
  ...
  <div class={'heading'}>Search all articles</div>
  <div id={'searchbox'}>
    <div class={'default'}><input class="ais-SearchBox-input" /></div>
  </div>
  <div id={'searchstats'}>
    <div class={'default'}>
      <span role="img" aria-label="emoji">🏃‍♀️</span> Working on retrieving results...
    </div>
  </div>
  <div id={'searchhits'} />
