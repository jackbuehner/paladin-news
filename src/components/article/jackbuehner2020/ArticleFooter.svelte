...
<div class="article-footer article-footer--buttons">
  ...
  {#if article.name}
    ...
    {@const facebook =
      share('facebook', article.name, article.description || '', articleLocation) ?? ''}
    {@const twitter =
      share('twitter', article.name, article.description || '', articleLocation) ?? ''}
    {@const email = share('email', article.name, article.description || '', articleLocation) ?? ''}
    {@const linkedin =
      share('linkedin', article.name, article.description || '', articleLocation) ?? ''}
