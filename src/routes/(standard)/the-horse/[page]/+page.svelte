...
<Container>
  ...
  <div class={'top-grid'} class:hidden={data?.page !== 1}>
    {#if data && data.docs && data.page === 1}
      {#each Array.from(data.docs).slice(0, 4) as article, index}
        <ArticleCard
  ...
  <div class={'main-grid'} class:firstPage={data?.page === 1}>
    {#if data && data.docs}
      {#each Array.from(data.docs).slice(data.page === 1 ? 4 : 0, 21) as article, index}
        {#if index !== 0}
