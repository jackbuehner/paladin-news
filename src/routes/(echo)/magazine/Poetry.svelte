<script lang="ts">
  ...
  export let poems: GET_ECHO_DOCS__DOC_TYPE[];
</script>

{#each insertDate(poems).slice(0, 6) as { name, authored_by, date, slug }}
  <article>
    <Name href="/magazine/poetry/{date.year}/{date.month}/{date.day}/{slug}">{name}</Name>
