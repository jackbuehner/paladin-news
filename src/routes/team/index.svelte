<script lang="ts">
  ...
  import type { GET_PROFILES__DOC_TYPE } from '$lib/queries';
  ...
  $: profiles = data ? (JSON.parse(data) as GET_PROFILES__DOC_TYPE[]) : undefined;

  $: boardProfiles = profiles?.filter((profile) => profile.group >= 1 && profile.group < 2) || [];
  $: managerProfiles = profiles?.filter((profile) => profile.group >= 2 && profile.group < 3) || [];
  $: photoVideoSocialProfiles =
    profiles?.filter((profile) => profile.group >= 3 && profile.group < 4) || [];
</script>
