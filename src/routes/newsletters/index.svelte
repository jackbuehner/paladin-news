  ...
  */
  function submitForm(event: MouseEvent) {
    ...
    if (email) {
      ...
      let formElemProcess = event.target as HTMLElement | null;
      while (!formElem && formElemProcess?.parentElement) {
        if (formElemProcess.parentElement.tagName === 'FORM') {
          formElem = formElemProcess.parentElement as HTMLFormElement;
        } else {
          formElemProcess = formElemProcess.parentElement;
        }
      }
      // set the email field and submit
      const emailFormElem: HTMLInputElement | undefined =
        formElem?.querySelector('[type=email]') || undefined;
      if (emailFormElem) emailFormElem.value = email;
