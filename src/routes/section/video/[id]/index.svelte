<script lang="ts">
  import type { GET_VIDEO__TYPE } from '$lib/queries';
  ...
  $: endpointData = data ? (JSON.parse(data) as GET_VIDEO__TYPE) : undefined;
  ...
  $: tokens = marked.lexer(endpointData?.videoPublic?.description || '');
  $: {
...
<article in:fly={{ y: 10, duration: 240 }}>
  ...
  <div class="plyr__video-embed" id="player">
    <iframe
      title={endpointData?.videoPublic?.name}
      src={(() => {
        if (endpointData?.videoPublic?.video_url) {
  ...
  <h1>{endpointData?.videoPublic?.name}</h1>
  <div class="description">
    <SvelteMarkdown source={marked.parser(tokens)} />
  </div>
  <div class="tags">
    {#each endpointData?.videoPublic?.tags || [] as tag}
      <span>#{tag}</span>
    {/each}
  </div>
  <p class={'navigation'}>
