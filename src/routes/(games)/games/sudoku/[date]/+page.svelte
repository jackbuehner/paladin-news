<script lang="ts">
  ...
  $: puzzle = data
    ? data[difficulty === 'challenge' ? 'challengePuzzle' : 'normalPuzzle'].map((row) =>
        row.split(',').map((num) => parseInt(num))
      )
    : undefined;

  $: date =
    data?.day && DateTime.fromISO(data.day).isValid
      ? DateTime.fromISO(data.day, { zone: 'utc' }).toISODate()
      : '';
  $: previous =
    data?.day && DateTime.fromISO(data.day).isValid && new Date(data.day) > new Date('2022-05-01')
      ? DateTime.fromISO(data.day, { zone: 'utc' }).minus({ days: 1 }).toISODate()
      : '';
  $: next =
    data?.day && DateTime.fromISO(data.day).isValid
      ? DateTime.fromISO(data.day, { zone: 'utc' }).plus({ days: 1 }).toISODate()
      : '';
