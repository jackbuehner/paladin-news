<script lang="ts">
  ...
  import type { GET_FRONT_PAGE_2022__TYPE } from '../queries';
    ...
    <div class={'grid'}>
      <section class={'col-left'} style="grid-area: c0">
        <div class="section-inner">
          ...
          {#each insertDate(endpointData?.news || []) as { name, slug, date, timestamps, photo_path }, index}
      ...
      <section class="col-right" style="grid-area: c2">
        <div class="section-inner">
          ...
          {#each insertDate(endpointData?.opinion || []) as { name, slug, date, people }}
          ...
          {#if endpointData?.videos?.[0]}
            <VideoCard
              name={endpointData.videos[0].name}
              thumbnail={(() => {
                const url = new URL(endpointData.videos[0].video_url);
    ...
    <div class={'grid'}>
      <section class={'col-left'} style="grid-area: c0">
        <div class="section-inner">
          ...
          {#each insertDate(endpointData?.sports || []) as { name, slug, date, timestamps, photo_path, categories }, index}
            {@const showPhoto = windowWidth <= 600 ? index === 0 : index < 2}
            <div class="side-col-item" class:padTop={index === 1}>
              <ArticleCard
                {name}
      ...
      <section class="col-right" style="grid-area: c2">
        <div class="section-inner">
          ...
          {#each insertDate(endpointData?.diversity || []) as { name, slug, date, timestamps, categories }}
            <div class="side-col-item">
              <ArticleCard
                {name}
                href={`/articles/${date.year}/${date.month}/${date.day}/${slug}`}
